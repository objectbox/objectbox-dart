  ...
  void _pushInt(int value, BitWidth width) {
    switch (width) {
      ...
      case BitWidth.width64:
        _setInt64Js(_buffer, _offset, value);
        break;
    }
  }
    ...
    if (ValueTypeUtils.isNumber(_type)) {
      if (_type == ValueType.Float) {
        ...
        switch (width) {
          ...
          case BitWidth.width64:
            final result = ByteData(8);
            _setInt64Js(result, 0, _value as int);
            return result.buffer.asUint8List();
        }
      }
    }
...
void _setInt64Js(ByteData buffer, int offset, int value) {
  final hi = (value / 4294967296).floor();
  final lo = value - hi * 4294967296;
  buffer.setUint32(offset, lo, Endian.little);
  buffer.setUint32(offset + 4, hi & 0xFFFFFFFF, Endian.little);
}
